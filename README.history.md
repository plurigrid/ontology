...
Both walk modes stop at a dead end unless `--restart` is supplied. A repository walk then teleports to an unvisited commit component; an organization walk teleports to an unvisited repository head. Empty repositories and repositories without a usable default-branch head remain in the catalog but cannot be selected for a walk. Parent lists and catalog pages fail closed if GitHub reports more records than the query fetched.
