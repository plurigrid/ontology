...
Organization refreshes fail closed when the reported repository total or organization identity changes between pages, when an overlapping repository row changes during pagination, or when pagination metadata is malformed. Repository refreshes likewise reject malformed pagination, history-count drift, changed commits or root snapshots between pages, and final default-history count mismatches. They compare the reported totals for branches, tags, pull requests, and each pull request's commits with the returned nodes; lists that exceed a 100-node GraphQL root window fail instead of producing a silently truncated cache. Commit parent lists must also fit in the requested GraphQL page. Supplying `--org` or `--repo` with an existing cache verifies that the cached identity matches; use `--refresh` to replace a mismatched cache.
